...
export function pushHistory(snap){
 ...
 HIST.u.push(snap);
 if(HIST.u.length>MAX)HIST.u.shift();
 HIST.r.length=0;
}
export const canUndo=()=>HIST.u.length>0;
export const canRedo=()=>HIST.r.length>0;
export const clearHistory=()=>{HIST.u.length=0;HIST.r.length=0};
...
export function undo(){
 ...
 HIST.r.push(cur);
 if(HIST.r.length>MAX)HIST.r.shift();
...
export function edit(fn){
 ...
 pushHistory(sn); touch(); AFTER(false); autosave();
 return out;
}
/* ═══ الحفظ التلقائي ═══
